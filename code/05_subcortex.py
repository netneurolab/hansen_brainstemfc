...
idx_subc = info.query("structure == 'subcortex'").index.values  # freesurfer subcortex
idx_dien = info.query("structure == 'diencephalon'").index.values  # brainstem navigator diencephalic nuclei
...
ax.scatter(*coords.T.values,
           s=strength["bd_bstem"]**2.1,  # 1.2 for schaefer400, 1.8 for schaefer100
           c=strength["bd_bstem"],
           cmap=PuBuGn_9.mpl_colormap,
           edgecolors=None)
ax.axis('off')
...
plt.savefig(path+'figures/eps/Schaefer' + str(parc) + '/pointbrain_bstemdien_hub_network_coronal.eps')
ax.view_init(0, 180)
plt.savefig(path+'figures/eps/Schaefer' + str(parc) + '/pointbrain_bstemdien_hub_network_sag.eps')
ax.view_init(90, 180)
plt.savefig(path+'figures/eps/Schaefer' + str(parc) + '/pointbrain_bstemdien_hub_network_axial.eps')
...
plot_subcortical(strength['subc_bstem'][enigma_reorder_idx],
                 ...
                 filename=path+'figures/eps/Schaefer' + str(parc) + '/surfacesubc_bstemhubs.png')
...
fig = plot_point_brain(assignments[idx, :67],  # don't plot fs subcortex
                       coords=info.query("structure == 'brainstem' or\
                                         structure == 'diencephalon'")[['x', 'y', 'z']].values,
...
qualcm = mpl.colors.ListedColormap(['#666666', '#196534', '#6d91cb', '#faa51b'])
...
plot_subcortical(assignments[idx, 67:][enigma_reorder_idx],
                 ...
                 size=(1800, 600), screenshot=True, transparent_bg=True,
                 filename=path+'figures/eps/Schaefer' + str(parc) + '/surfacesubc_communities_' + str(idx) + '.png')
...
plt.tight_layout()
plt.savefig(path+'figures/eps/Schaefer' + str(parc) + '/heatmap_fcregcorr-notctx.eps')

# brainstem gradient
fc_grad_notctx, u, s, v = diffusion_map_embed(np.corrcoef(fc_reg[:, idx_ctx].T),
                                              no_dims=3, alpha=0.5)

brain = plot_fsaverage(data=fc_grad_notctx[:, 0],  # why is plot_fsaverage broken?!
                       lhannot=annot.lh, rhannot=annot.rh,
                       colormap=cmap,
...
plot_subcortical(data[67:][enigma_reorder_idx],
                 ventricles=False, cmap='PuBuGn', color_range=(np.min(data), np.max(data)),
                 size=(1800, 600), screenshot=True, transparent_bg=True,
                 filename=path+'figures/eps/Schaefer' + str(parc) + '/surfacesubc_grad_neg.png')
...
plot_subcortical(data[67:][enigma_reorder_idx],
                 ventricles=False, cmap='PuRd', color_range=(np.min(data), np.max(data)),
                 size=(1800, 600), screenshot=True, transparent_bg=True,
                 filename=path+'figures/eps/Schaefer' + str(parc) + '/surfacesubc_grad_pos.png')
